 ...
 * @file     main.c
 *
 * @brief    Demonstrate PDMA2 channel 1 get/clear timeout flag with UART5.
 *
...
void SYS_Init(void)
{
	...
	SYS->GPA_MFPH |= (SYS_GPA_MFPH_PA10MFP_UART5_RXD | SYS_GPA_MFPH_PA11MFP_UART5_TXD);
...
{
	/* Open PDMA2 Channel */
	PDMA_Open(PDMA2,1 << 0); // Channel 0 for UART5 TX
	PDMA_Open(PDMA2,1 << 1); // Channel 1 for UART5 RX
	// Select basic mode
	PDMA_SetTransferMode(PDMA2,0, PDMA_UART5_TX, 0, 0);
	PDMA_SetTransferMode(PDMA2,1, PDMA_UART5_RX, 0, 0);
	...
	PDMA_SetTransferAddr(PDMA2,0, (ptr_to_u32((&g_u8Tx_Buffer[0]))), PDMA_SAR_INC, UART5_BASE, PDMA_DAR_FIX);
	PDMA_SetTransferAddr(PDMA2,1, UART5_BASE, PDMA_SAR_FIX, (ptr_to_u32(&g_u8Rx_Buffer[0])), PDMA_DAR_INC);
...
void UART_PDMATest()
{
	...
	sysprintf("|    Please connect UART5_TX and UART5_RX pin.              |\n");
	sysprintf("+-----------------------------------------------------------+\n");
	...
	/*
		Using UART5 external loop back.
		This code will send data from UART5_TX and receive data from UART5_RX.
		UART5_TX :  Total transfer length =  (PDMA_TEST_LENGTH  ) * 8 bits
		UART5_RX :  Total transfer length =  (PDMA_TEST_LENGTH+1) * 8 bits
	*/
	...
	UART5->INTEN |= UART_INTEN_TXPDMAEN_Msk | UART_INTEN_RXPDMAEN_Msk;

	while(u32IsTxTestOver == 0);

	if (u32IsTxTestOver == 1)
		sysprintf("UART5 TX transfer done...\n");
	else if (u32IsTxTestOver == 2)
		sysprintf("UART5 TX transfer abort...\n");
	else if (u32IsTxTestOver == 3)
		sysprintf("UART5 TX timeout...\n");

	while(u32IsRxTestOver == 0);

	if (u32IsRxTestOver == 1)
		sysprintf("UART5 RX transfer done...\n");
	else if (u32IsRxTestOver == 2)
		sysprintf("UART5 RX transfer abort...\n");
	else if (u32IsRxTestOver == 3){
		sysprintf("UART5 RX timeout...\n");
	}
